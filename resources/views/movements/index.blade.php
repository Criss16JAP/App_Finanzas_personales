                    ...
                    <form action="{{ route('movements.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex space-x-4">
                                <label class="flex items-center"><input type="radio" name="type" value="egress" class="mr-2" checked>Gasto</label>
                                <label class="flex items-center"><input type="radio" name="type" value="income" class="mr-2">Ingreso</label>
                                <label class="flex items-center"><input type="radio" name="type" value="transfer" class="mr-2">Transferencia</label>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <div id="account_id_wrapper">
                                    ...
                                    <select name="account_id" id="account_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                ...
                                <div id="egress_category_wrapper" class="mt-4">
                                    ...
                                    <select name="category_id" id="egress_category_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="amount" id="amount" step="0.01" min="0.01" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                </div>
                            </div>
                            <div>
                                <div id="related_account_id_wrapper" class="hidden">
                                    ...
                                    <select name="related_account_id" id="related_account_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                        <option value="">Selecciona una cuenta</option>
                                        @foreach ($accounts as $account)
                                            <option value="{{ $account->id }}">{{ $account->name }} (${{ number_format($account->balance, 2) }})</option>
                                        @endforeach
                                ...
                                <div id="income_category_wrapper" class="hidden mt-4">
                                    ...
                                    <select name="category_id" id="income_category_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                        <option value="">Selecciona una categoría</option>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="movement_date" id="movement_date" value="{{ date('Y-m-d') }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                </div>
                                <div class="mt-4">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($movements as $movement)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $movement->movement_date->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4">{{ $movement->description ?? '--' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $movement->account->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $movement->category->name ?? '--' }}</td>
